...
{{config(
    ...
    unique_key=['event_date','product_id','brand'],
...
SELECT
    ...
    fct_events.brand,
    CAST(SUM(fct_events.price) AS NUMBER(18,2)) total_sum,
...
JOIN {{ref('ctl_dates_to_process')}} ctl_dates USING(event_date)
{% endif %}
WHERE fct_events.event_type = 'purchase'
GROUP BY fct_events.event_date, fct_events.product_id, fct_events.brand